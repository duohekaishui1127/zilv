...
- 倒计时在前台或后台归零时都会结束计时并写入消息中心；本次已授权时由后台发送微信订阅消息。前台同时振动提示。倒计时到点不会自动完成任务或触发整日打卡。
- 正计时首次开始时准备一次提醒额度；累计有效计时达到 2.5 小时仍在运行时，前台任一底部导航页轻振动并提示休息，前后台均写入消息中心，已授权时由定时调度发送微信提醒。计时不会自动停止；未授权时仍保留站内记录。
- 离开小程序后不持续运行前台计时器，也不在手机状态栏或锁屏展示实时秒数；重新进入时按计时记录恢复。后台不能自行播放提示音或触发手机振动，微信通知的提示方式由系统设置决定。
- 正计时任务由用户手动结束并完成。若 2.5 小时提醒已消耗额度，完成点击会申请下一次正计时提醒；未消耗的额度直接保留，不重复弹窗。
- 运行中的计时会在五个底部导航页顶部持续显示；跨天后仍可回到“今日”暂停、继续或结束，时长和完成记录归入启动日。跨天到点但未完成的计时在之后 7 天提供待完成入口。
...
9. 启动一个短倒计时并同意订阅：前台到点会振动、写入消息中心并等待后台发送本次订阅消息；再次启动后退到后台，到点后也会有消息中心记录和已授权的微信提醒。两次都需用户自行完成任务。
10. 启动正计时并同意订阅，构造累计有效时间达到 2.5 小时：在五个底部导航页分别确认前台轻振动、站内消息且继续计时；退出小程序后确认后台也写入消息中心并发送已授权的休息建议订阅消息。跨天计时同样只提醒一次。手动结束任务时确认已消耗额度会续订、未消耗额度不会重复申请。
