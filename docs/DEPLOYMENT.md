...
5. 调用 `admin-init` 一次，确认 `success: true`、`appVersion: 1.4.0`、`schemaVersion: 7`。
...
14. 编译运行并在“我的 → 关于与诊断”确认客户端/服务端版本均为 1.4.0、Schema 均为 7。
...
- 确认 29 个集合、种子数据和七条 Schema migration 已就绪；
- 再部署 `api`；
