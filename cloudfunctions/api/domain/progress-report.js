...
function buildDailySeries({ dates, bodies = [], mealItems = [], workouts = [], studySessions = [], checkins = [], targets = [] }) {
  ...
  return dates.map(date => {
    ...
    const activityScore = Math.min(4,
      ...
      (number(study.durationMinutes) > 0 ? 1 : 0) + (number(checkin.completed) > 0 ? 1 : 0) + (body ? 1 : 0))
    return {
      date,
...
module.exports = { dateRange, monthRange, buildDailySeries, buildProgressReport, buildActivityCalendar }
