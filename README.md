...
- 好友：好友码、带选填备注的申请处理与红点通知；好友资料以当月日历展示获准公开的任务进度，支持本地备注、全局隐私默认、单好友权限例外与“特别关心”。
- 群组：支持复制群组 ID、邀请、计划群组绑定和打卡动态；成员按未完成比例排序，可查看仅含群组任务进度的当月打卡日历。
...
运行 `admin-init` 后会自动创建当前版本所需的 **32 个集合**，并幂等写入：
